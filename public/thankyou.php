...
if($response['payments'][0]['status']=='Credit'){
    ...
    $pname = $response['payments'][0]['buyer_name'];
     ...
     $data = "SELECT * FROM tbl_idma_2018_deluserpayment where email='$email'";
    $result = $conn->query($data);
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                $id = $row["ID"];
                  $sql = "UPDATE tbl_idma_2018_deluserpayment SET txn_id='$payment_id', statuss='1' WHERE id=$id";
...
$mail->Subject = 'Order No. '.$row["orderid"].' FROM  Delegate IDMA 2018';
...
<strong>Please Note:</strong>- The charge will appear on your credit card as "Adsert Web Solutions Pvt. Ltd.". If you have a question about your order status, you can contact directly by e-mail at : <a href="mailto:nikita.vig@exchange4media.com" style="color:#002467;">nikita.vig@exchange4media.com</a>.</p>
  ...
  <tr>
    ...
    <td style="background:#f2f5f8;">'.$row["OrderNo"].'</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td style="background:#f2f5f8;">'.$row["pdate"].'</td>
  </tr>
  <tr>
    <td style="background:#f2f5f8;">Delegate Pass Category:</td>
    <td style="background:#f2f5f8;">'.$row["delegate_Pass_category"].'</td>
  </tr>
  <tr>
    <td style="background:#f2f5f8;">Delegate Type</td>
    <td style="background:#f2f5f8;">'.$row["Delegate_Type"].'</td>
  </tr>
  <tr>
    <td style="background:#f2f5f8;">Total Amount</td>
    <td style="background:#f2f5f8;">'.$row["Amount"].'</td>
  </tr>
</table>
<tr><td>
<table width="100%" border="0" cellspacing="4" cellpadding="5">
  ...
  <tr>
    ...
    <td style="background:#f2f5f8;">'.$row["Organisation"].' </td>
  </tr>
  <tr>
    <td style="background:#f2f5f8;">GST IN</td>
    <td style="background:#f2f5f8;">'.$row["gst"].' </td>
  </tr>
  <tr>
    <td style="background:#f2f5f8;">Contact Number</td>
    <td style="background:#f2f5f8;">+'.$row["MobileNo"].'</td>
  </tr>
  <tr>
    <td style="background:#f2f5f8;">E-mail ID: </td>
    <td style="background:#f2f5f8;">'.$row["Email"].'</td>
  </tr>
</table>
    </td></tr>
    <tr>
